...
namespace ECE
{
  public static class EasyColliderSaving
  {
    ...
    public static string GetValidConvexHullPath(GameObject go)
    {
      ...
      string path = ECEPreferences.SaveConvexHullPath + string.Join("_", go.name.Split(Path.GetInvalidFileNameChars()));
      // get path to go
      ...
      if (badPath || !Directory.Exists(directory) || !AssetDatabase.IsValidFolder(assetDatabaseFolder))
      {
        ...
        if (fullPathIndex >= 0)
        {
          ...
          if (ECEPreferences.SaveConvexHullPath.Contains("EasyColliderEditor"))
          {
            ...
            if (ECEPreferences.ResetDefaultSavePath())
            {
          ...
          fullPath = fullPath + "/" + string.Join("_", go.name.Split(Path.GetInvalidFileNameChars()));
        }
        else
        {
          ...
          fullPath = Application.dataPath + "/" + string.Join("_", go.name.Split(Path.GetInvalidFileNameChars()));
        }
      }
      return fullPath;
    }
